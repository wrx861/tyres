...
function App() {
  ...
  const renderPage = () => {
    switch (currentPage) {
      ...
      case 'search':
        return <SearchPage user={user} cart={cart} onAddToCart={addToCart} onRemoveFromCart={removeFromCart} onBack={() => setCurrentPage('home')} />;
      case 'car-selection':
        return <CarSelectionPage user={user} cart={cart} onAddToCart={addToCart} onRemoveFromCart={removeFromCart} onBack={() => setCurrentPage('home')} />;
      case 'cart':
        return (
          <CartPage 
            cart={cart}
  ...
  return (
    <WarehousesContext.Provider value={warehouses}>
      <div className="min-h-screen bg-gray-50 dark:bg-gray-900 transition-colors duration-200">
        {/* Кнопка переключения темы (только на главной странице) */}
        {currentPage === 'home' && (
          <button
