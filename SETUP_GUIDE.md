...
**Интерактивный режим (рекомендуется для первой установки):**
```bash
...
> **Важно:** Не используйте `wget -qO- ... | bash` для интерактивной установки, так как это не позволит вводить данные!

### Шаг 3: Настройка (интерактивный режим)

Если вы запустили установщик без параметров, он задаст вопросы:
...
4. **Логин 4tochki API** - ваш логин от b2b.4tochki.ru
5. **Пароль 4tochki API** - ваш пароль
6. **Telegram Bot Token** - получите от [@BotFather](https://t.me/BotFather)
7. **Admin Telegram ID** - получите от [@userinfobot](https://t.me/userinfobot)

> **Важно:** Если вы пропустите учетные данные (нажмете Enter), вы сможете добавить их позже в файле `/opt/tyres-app/backend/.env`

### Примеры команд установки

**1. Полная продакшн установка с SSL:**
```bash
wget https://raw.githubusercontent.com/wrx861/tyres/main/install.sh
sudo bash install.sh -d tires.myshop.ru -e admin@myshop.ru
```

**2. Установка с доменом, SSL настроим позже:**
```bash
sudo bash install.sh -d tires.myshop.ru --no-ssl
```

**3. Локальная разработка (без домена):**
```bash
sudo bash install.sh
# Просто нажмите Enter когда спросит про домен
```
...
Если вы ввели учетные данные во время установки, они уже настроены! ✅
...
Убедитесь что заполнены необходимые данные:
